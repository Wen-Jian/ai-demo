...
def cnn_with_batch_norm(x_s, k_size, input_features, output_features, strides):
    y1 = cnn.add_cnn_layer(x_s, [k_size[0], k_size[1], in_channel, output_features], strides=strides)
...
def train_heigh_resolution_with_gpu(datasets, batch_size, input_shape, output_shape, channel_size, sess):
    ...
    with tf.device('/device:GPU:0'):
        ...
        # stage 1
        y1 = cnn_with_batch_norm(x_s, [7, 7], 3, 64)

        y2 = cnn_with_batch_norm(y1, [1, 1], 64, 64)

        y3 = cnn_with_batch_norm(y2, [3, 3], 64, 64)

        y4 = cnn_with_batch_norm(y3, [3, 3], 64, 64)

        y5 = cnn_with_batch_norm(y4, [3, 3], 64, 256)

        # stage 1 x
        x_ = cnn_with_batch_norm(x_s, [1,1], 3, 256)
